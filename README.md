...
**分段质量**（7 个测试文档，共 211 个 chunk）：

| 指标 | 目标 | 实际 | 达标 |
|------|------|------|------|
| 表格/公式/代码整体成块率 | ≥95% | 100% | ✅ |
| 目标长度区间命中率 | ≥90% | 97~100% | ✅ |
| 原文回链完整率 | 100% | 100% | ✅ |

**RAG 检索提升**（Smart vs 固定长度 Baseline）：

| 指标 | 智能分段 | 固定基线 | 提升 |
|------|---------|---------|------|
| Recall@1 | 0.162 | 0.146 | +11.5% |
| Recall@3 | 0.458 | 0.301 | +52.5% |
| Recall@5 | 0.565 | 0.491 | +15.1% |
| nDCG@5 | 0.678 | 0.594 | +14.1% |
| MRR | 0.756 | 0.750 | +0.7% |

> Recall@5 提升 +15.1%，超过任务书要求的 ≥10%。
...
| 项目 | 说明 |
|------|------|
| 不破句率检测 | 代码中无破句检测逻辑，需在 statistics.py 中实现 |
| 语义完整性人工评估 | `scripts/human_eval_semantic.py` 已就绪，待执行抽样打分 |
| 标签准确率/摘要忠实度评测 | `evaluator.py` 已恢复，待对 3 文档数据集运行完整评测 |
| QA 合成评测 | `synthesize_qa_pairs.py` 依赖已恢复（需 API Key），待运行并评测可答性/忠实度 |
| 长表格按行组拆分 | 已列入计划 |
| Docker/部署文档 | 未开始 |
| 正式技术报告 | 未开始 |
...
| 指标 | 目标 | 当前 | 状态 | 说明 |
|------|------|------|------|------|
| 表格/公式/代码整体成块率 | ≥95% | 100% | ✅ | 有代码支撑 |
| 目标长度区间命中率 | ≥90% | 97~100% | ✅ | 有代码支撑 |
| Recall@k/nDCG 提升 | ≥10% | +15.1% | ✅ | eval_rag.py 可运行 |
| 原文回链完整率 | 100% | 100% | ✅ | 有代码支撑 |
| 不破句率 | 100% | **无法检测** | ❌ | 代码中无破句检测逻辑 |
| 语义完整性（人工评估） | ≥85% | **未执行** | ❌ | human_eval_semantic.py 已就绪 |
| 标签准确率 | ≥85% | **未评测** | ❌ | evaluator.py 已恢复，待运行 |
| 摘要忠实度 | ≥90% | **未评测** | ❌ | evaluator.py 已恢复，待运行 |
| QA 合成可答性 | ≥90% | **未评测** | ❌ | synthesize_qa_pairs.py 依赖已恢复 |
| QA 合成忠实度 | ≥90% | **未评测** | ❌ | synthesize_qa_pairs.py 依赖已恢复 |
...
- LLM 生成式标签/摘要的评估方法（embedding 余弦相似度）对改写措辞天然低估，导致指标偏低；实际质量可能优于指标所示
- 不破句率检测规则偏严（非句末标点结尾均判破句）
- `backend/app/services/rag_store/` 与 `backend/app/services/retrieval/` 存在功能重叠（均为检索），后续可整合
- 前端 RetrivalPanel 已添加到组件目录，但未集成到 HomeView（需手动引用）
