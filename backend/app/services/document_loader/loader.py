...
def load_document(
    ...
    doc_id 和 image_dir 用于文档内图片的提取与存储。
    """
    ...
    if suffix == ".docx":
        return load_docx_file(path, doc_id=doc_id, image_dir=image_dir)
    if suffix == ".pdf":
        return load_pdf_file(path, doc_id=doc_id, image_dir=image_dir)
